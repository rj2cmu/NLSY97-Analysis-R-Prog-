...
nlsy.recode <- nlsy.recode %>%
  mutate(MaritalStatus_2017 = recode_factor(MaritalStatus_2017,
                                `0` = "Never-married",
                                `1` = "Married",
                                `2` = "Separated",
                                `3` = "Divorced",
...
```
```{r, fig.height = 4, fig.width = 8}
ggplot(data = nlsy.recode, aes(x = Income, y = Highest_Grade_Completed_2011 , colour = Industry_Code_2017 )) + geom_point() + stat_smooth(method = 'loess', formula = y ~ x, size = 1) + theme(axis.text.x=element_text(angle =- 90, vjust = 0.5)) + ylim(0,22)
```
```{r}
...
#No. of Biological Children at home
...
ggplot(data = num.bio.child, aes(x = Children_HH_2009, y = average.income, fill = average.income)) + 
...
```{r}
num.bio.child <- nlsy.recode %>%
  group_by(Children_HH_2009, na.rm = TRUE) %>%
