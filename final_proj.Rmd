...
<font color="#157515">
In the previous boxplot, we can see that the the average income is slightly higher for those who hadn't consumed cocaine by 1998, but we still don't know if it statistically sifnificant. 
</font>
```{r}
...
<font color="#157515">
The coefficient values `r round(abs(coef(nlsy.lm.interact.hh.worth))["HH_NET_WORTH_30"]),0)` are so small that we don't want to further in this analysis.
...
```
<font color="#157515">
Considering again whether the average income somewhat determines household worth, having filtered the topcoded incomes, with the linear model, we can see that in average females have a larger household worth for the same income levels, without establishing this is statistically significant.
</font>
```{r}
