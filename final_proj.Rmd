...
nlsy.recode <- nlsy %>%
  mutate(Sex = recode_factor(Sex,
                                ...
                                `2` = "Female",
                                `0` = "No Information"))
nlsy.recode <- select(nlsy.recode, Marijuana_1997, "%Ch.Dip.20_1997", "%Ch.Baby_1997", "%Ch.Coll.Degree.30_1997", Sex, BDATE_M_1997, BDATE_Y_1997, Dep_F_1997, Dep_M_1997, HH_NET_WORTH_P_1997, 
...
nlsy.recode <- filter(nlsy.recode, !is.na(Income), !is.na(MaritalStatus_2017))
...
```
```{r}
lm.income.sex.marital <- lm(Income ~ Sex + MaritalStatus_2017,
                            data = nlsy.recode)
...
#anova test for comparing income + marital status & income + sex + marital status
